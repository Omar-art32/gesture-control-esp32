...
namespace LedRGB {
    ...
    // ── Colores predefinidos para cada estado del sistema ─────────
    struct Color {
        uint8_t rojo;
        uint8_t verde;
        uint8_t azul;
    ...
    constexpr Color COLOR_CONECTADO  = VERDE;      // BT emparejado
    constexpr Color COLOR_ESPERANDO  = AZUL;       // esperando gesto
    constexpr Color COLOR_GESTO      = AMARILLO;   // gesto detectado
    constexpr Color COLOR_ERROR      = ROJO;       // fallo de conexión
    ...
    // ── Parpadeo bloqueante (para notificaciones cortas) ─────────
